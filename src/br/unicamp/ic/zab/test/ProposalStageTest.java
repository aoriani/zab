...
public class ProposalStageTest {
    /**
    ...
    /**
     * Test if Proposal stage can finish after just have started and if it shutdowns
     * the next stage
     * @throws IOException
     * @throws InterruptedException
     */
    ...
    public void testSimpleShutdown() throws IOException, InterruptedException{
        ...
        ProposalStage stage = new ProposalStage(new Leader(null), dummyStage);
        stage.start();
        Thread.sleep(1000);
        stage.shutdown();
        stage.join();
    ...
    /**
     * Test if Proposal stage can finish if interrupt is called and if in that cases
    ...
    public void testDeliverTwoPackets() throws IOException, InterruptedException{
        ...
        new MockUp<Leader>(){
            ...
            public void sendPacketToFollowers(Packet packet){
                assertEquals("Packet should be a PROPOSAL",Packet.Type.PROPOSAL, packet.getType());
                ++times;
                switch(times){
                    case 1:
